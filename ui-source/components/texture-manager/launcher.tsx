...
type Lang = "es" | "en"
type Tab = "sky" | "textures" | "fonts" | "potato" | "flags" | "presets"
...
const TR = {
  es: {
    ...
    presetsTitle: "Presets Cloud", presetsSub: "Comparte y descarga configuraciones de la comunidad",
    presetsPublic: "Públicos", presetsMine: "Mis presets",
    searchPreset: "Buscar preset...", noPresets: "No hay presets disponibles",
    downloadPreset: "Descargar", likePreset: "Me gusta", reportPreset: "Reportar",
    createPreset: "Crear preset", editPreset: "Editar", deletePreset: "Eliminar",
    presetName: "Nombre del preset", presetDesc: "Descripción",
    presetTags: "Etiquetas (separadas por coma)", presetVisibility: "Visibilidad",
    visibilityPublic: "Público", visibilityPrivate: "Privado",
  },
  en: {
    ...
    presetsTitle: "Presets Cloud", presetsSub: "Share and download community configurations",
    presetsPublic: "Public", presetsMine: "My presets",
    searchPreset: "Search preset...", noPresets: "No presets available",
    downloadPreset: "Download", likePreset: "Like", reportPreset: "Report",
    createPreset: "Create preset", editPreset: "Edit", deletePreset: "Delete",
    presetName: "Preset name", presetDesc: "Description",
    presetTags: "Tags (comma separated)", presetVisibility: "Visibility",
...
export function Launcher() {
  ...
  const [presetView, setPresetView] = useState<"public" | "mine">("public")
  ...
  useEffect(() => {
    async function init() {
      ...
      // Load user profile from app config
      const config = await electronAPI.loadAppConfig()
      if (config.success && config.config) {
        const { robloxUsername, robloxAvatar, robloxFriendCount, robloxDescription } = config.config as any
        if (robloxUsername) {
          setUserProfile({
            username: robloxUsername,
            avatar: robloxAvatar || "https://tr.rbxcdn.com/30DAY-AvatarHeadshot-695CED98FDC232201477E9A144B99CE4-Png/150/150/AvatarHeadshot/Webp/noFilter",
            friendCount: robloxFriendCount || 0,
  ...
  // Load presets when tab changes to presets
  useEffect(() => {
    async function loadPresets() {
      ...
      try {
        const config = await electronAPI.loadAppConfig()
        if (config.success && config.config) {
          const userId = (config.config as any).userId
          if (presetView === "mine" && userId) {
            const result = await electronAPI.getUserPresets(userId)
            if (result.success) {
              setPresets(result.presets || [])
            }
          } else {
            const result = await electronAPI.getPresets("public", 50, 0)
            if (result.success) {
              setPresets(result.presets || [])
            }
          }
  ...
  const tabContent: Record<Tab, React.ReactNode> = {
    ...
    presets: (
      <div className="flex flex-col gap-3 h-full">
        <p className="text-[11px] text-[#888580] uppercase tracking-widest font-medium">{tr.presetsTitle}</p>
        <div className="flex gap-2">
          <button
            onClick={() => setPresetView("public")}
            className={`flex-1 rounded-lg px-3 py-2 text-xs font-medium transition-all ${presetView === "public" ? "bg-[#252320] text-white" : "text-[#888580] hover:bg-[#252320]/50"}`}
          >
            {tr.presetsPublic}
          </button>
          <button
            onClick={() => setPresetView("mine")}
            className={`flex-1 rounded-lg px-3 py-2 text-xs font-medium transition-all ${presetView === "mine" ? "bg-[#252320] text-white" : "text-[#888580] hover:bg-[#252320]/50"}`}
          >
            {tr.presetsMine}
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 space-y-2 pr-1">
          {loadingPresets ? (
            <div className="flex items-center justify-center py-8">
              <Loader2 className="h-5 w-5 animate-spin text-[#888580]" />
            </div>
          ) : presets.length === 0 ? (
            <div className="flex items-center justify-center py-8">
              <p className="text-sm text-[#888580]">{tr.noPresets}</p>
            </div>
          ) : (
            presets.map((preset: any) => (
              <div key={preset.id} className="rounded-lg border border-[#2E2C28] bg-[#252320]/50 p-3">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[#AEAEAE] truncate">{preset.name}</p>
                    <p className="text-[11px] text-[#888580] truncate">{preset.description}</p>
                  </div>
                  <div className="flex items-center gap-1 ml-2">
                    <Heart className="h-3.5 w-3.5 text-[#888580]" />
                    <span className="text-[11px] text-[#888580]">{preset.likes_count || 0}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={async () => {
                      try {
                        ...
                        const presetResult = await electronAPI.getPreset(preset.id);
                        if (!presetResult.success || !presetResult.preset) {
                          toast.error("Error al obtener preset");
                          return;
                        }

                        const presetData = presetResult.preset;
                        const configJson = typeof presetData.config_json === 'string'
                          ? JSON.parse(presetData.config_json)
                          : presetData.config_json;

                        // Apply the preset configuration
                        if (configJson.skybox && texturePath) {
                          await electronAPI.applySky(configJson.skybox, texturePath);
                        }

                        if (configJson.darkTextures !== undefined && texturePath) {
                          await electronAPI.applyDarkTextures(configJson.darkTextures, texturePath);
                        }

                        if (configJson.flags) {
                          await api?.saveFlags?.(configJson.flags);
                          setFlags(configJson.flags);
                        }

                        if (configJson.potatoMode) {
                          await api?.getPotatoFlags?.().then((fr: any) => {
                            if (fr?.success) {
                              api?.saveFlags?.(fr.flags);
                              setFlags(fr.flags);
                            }
                          });
                          setPotatoOn(true);
                        ...
                        // Save the applied preset to app config
                        await electronAPI.saveAppConfig({
                          selectedSky: configJson.skybox || null,
                          darkOn: configJson.darkTextures || false,
                          potatoOn: configJson.potatoMode || false
                        });

                        // Download to increment counter
                        await electronAPI.downloadPreset(preset.id);
                        toast.success("Preset aplicado correctamente");
                      } catch (error) {
                        console.error('Error applying preset:', error);
                        toast.error("Error al aplicar preset");
                      }
                    }}
                    className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-[#252320] px-2 py-1.5 text-[11px] text-[#AEAEAE] hover:bg-zinc-700 transition-all"
                  >
                  ...
                  <button
                    onClick={async () => {
                      ...
                      if (config.success && config.config) {
                        const userId = (config.config as any).userId;
                        if (userId) {
                          const result = await electronAPI.likePreset(preset.id, userId);
                          if (result.success) {
                            // Refresh presets
                          }
                        }
                    ...
                    className="rounded-lg border border-[#2E2C28] px-2 py-1.5 text-[#888580] hover:text-[#AEAEAE] hover:border-[#3A3835] transition-all"
                  >
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#1A1A1E]" style={{ fontFamily: "'gg sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', system-ui, sans-serif" }}>
      <AnimatePresence mode="wait">
        ...
        {view === "home" && (
          <motion.div key="home"
            ...
            {/* Header */}
            <div className="flex items-center gap-3 border-b border-[#232328] px-5 py-3.5">
              <img
              ...
              <span className="ml-1 rounded-md bg-[#111214] px-2 py-0.5 text-[10px] font-medium text-[#F2F3F5]">v{appVersion}</span>
              <div className="ml-auto flex items-center gap-2">
                <button onClick={() => setLang(lang === "es" ? "en" : "es")}
                  className="rounded-md bg-[#111214] px-2.5 py-1 text-[11px] font-semibold text-[#F2F3F5] hover:text-[#F2F3F5] hover:bg-zinc-700 transition-all">
                  {lang.toUpperCase()}
                </button>
              </div>
            </div>
            ...
            <div className="flex">
              {/* Left */}
              <div className="flex w-44 flex-col border-r border-[#232328] p-5">
                <img
                  src={userProfile?.avatar || "https://tr.rbxcdn.com/30DAY-AvatarHeadshot-695CED98FDC232201477E9A144B99CE4-Png/150/150/AvatarHeadshot/Webp/noFilter"}
                  alt={userProfile?.username || "YUMMAN"} className="mb-4 h-14 w-14 rounded-xl object-cover border border-[#232328]" crossOrigin="anonymous" />
                <p className="text-sm font-semibold text-[#F2F3F5]">{userProfile?.username || "YUMMAN"}</p>
                <p className="text-[11px] text-[#B5BAC1] mt-0.5 mb-auto">
                  {userProfile?.friendCount !== undefined ? `${userProfile.friendCount} friends` : "Rivals Launcher"}
                </p>
                {userProfile?.description && (
                  <p className="text-[10px] text-[#888580] mt-1 mb-auto line-clamp-2">{userProfile.description}</p>
                )}
                <div className="flex flex-col gap-2 mt-6">
                  <button onClick={() => ext("https://discord.com/invite/EVWqd5swAt")}
                    className="flex items-center gap-2 text-[11px] text-[#B5BAC1] hover:text-[#F2F3F5] transition-colors">
              ...
              <div className="flex flex-1 flex-col gap-2 p-5">
                ...
                <button onClick={handleLaunch} disabled={launchState === "launching"}
                  className={`group flex items-center justify-between rounded-xl px-4 py-3.5 transition-all hover:scale-[1.01] disabled:cursor-not-allowed ${
                    launchState === "done" ? "bg-[#111214] border border-[#393A41]" :
                    launchState === "error" ? "bg-[#111214] border border-[#232328]" :
                    "bg-[#111214] border border-[#232328] hover:border-[#393A41] hover:bg-[#232328]"
                  }`}>
                  <div className="flex items-center gap-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#5865F2]/5">
                      {launchState === "launching" ? <Loader2 className="h-4 w-4 animate-spin text-white" /> :
                       launchState === "done" ? <Check className="h-4 w-4 text-[#23A55A]" /> :
                       launchState === "error" ? <X className="h-4 w-4 text-[#F2F3F5]" /> :
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-[11px] text-[#B5BAC1]">{tr.launchSub}</p>
                    </div>
                  </div>
                  <ArrowRight className="h-4 w-4 text-[#B5BAC1] group-hover:text-[#F2F3F5] transition-colors" />
                </button>

                {/* Settings */}
                <button onClick={goSettings}
                  className="group flex items-center justify-between rounded-xl border border-[#232328] bg-[#111214] px-4 py-3.5 transition-all hover:scale-[1.01] hover:border-[#393A41] hover:bg-[#232328]">
                  <div className="flex items-center gap-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#5865F2]/5">
                      <Settings className="h-4 w-4 text-white" />
                    </div>
                    <div className="text-left">
                      <p className="text-sm font-semibold text-[#F2F3F5]">{tr.configure}</p>
                      <p className="text-[11px] text-[#B5BAC1]">{tr.configureSub}</p>
                    </div>
                  </div>
                  <ArrowRight className="h-4 w-4 text-[#B5BAC1] group-hover:text-[#F2F3F5] transition-colors" />
                </button>
                ...
                <button onClick={handleInstall} disabled={isInstalling}
                  className="group flex items-center gap-3 rounded-xl border border-[#232328] px-4 py-2.5 transition-all hover:border-[#393A41] hover:bg-[#111214] disabled:opacity-40">
                  {isInstalling ? <Loader2 className="h-3.5 w-3.5 animate-spin text-[#B5BAC1]" /> : <Download className="h-3.5 w-3.5 text-[#B5BAC1] group-hover:text-[#F2F3F5] transition-colors" />}
                  <p className="text-xs text-[#B5BAC1] group-hover:text-[#F2F3F5] transition-colors">
                ...
                <button onClick={handleCheckUpdate} disabled={isCheckingUpdate}
                  className="group flex items-center gap-3 rounded-xl border border-[#232328] px-4 py-2.5 transition-all hover:border-[#393A41] hover:bg-[#111214] disabled:opacity-40">
                  {isCheckingUpdate ? <Loader2 className="h-3.5 w-3.5 animate-spin text-[#B5BAC1]" /> : <Download className="h-3.5 w-3.5 text-[#B5BAC1] group-hover:text-[#F2F3F5] transition-colors" />}
                  <p className="text-xs text-[#B5BAC1] group-hover:text-[#F2F3F5] transition-colors">
                ...
                {(selectedSky || darkOn || potatoTexOn || lowLatOn || potatoOn) && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {selectedSky && <Badge icon={<Cloud className="h-3 w-3 text-[#F2F3F5]" />} label={skyboxes.find(s => s.id === selectedSky)?.[lang === "es" ? "name" : "nameEn"] ?? selectedSky} />}
                    {darkOn && <Badge icon={<Moon className="h-3 w-3 text-[#F2F3F5]" />} label="Dark" />}
            ...
            <div className="flex flex-1 min-h-0">
              ...
              <nav className="flex w-36 flex-col gap-0.5 border-r border-[#2E2C28] p-2">
                {([
                  ...
                  { id: "presets" as Tab, icon: Cloud, label: tr.presetsTitle },
                ]).map(t => {
                  const Icon = t.icon
