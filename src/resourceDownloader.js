...
class ResourceDownloader {
  constructor() {
    // URL dinámica basada en la versión de la app
    const appVersion = app.getVersion();
    this.resourcesUrl = `https://github.com/mmgb5656/yumman-rivals/releases/download/v${appVersion}-resources/yumman-rivals-resources-v${appVersion}.zip`;
  ...
  async checkDiskSpace(requiredBytes) {
    try {
      const stats = await fs.statfs(app.getPath('temp'));
      const freeSpace = stats.free;
      const requiredSpace = requiredBytes * 1.5; // 50% de overhead
      ...
      log.info(`Espacio disponible: ${Math.round(freeSpace / 1024 / 1024)} MB, Requerido: ${Math.round(requiredSpace / 1024 / 1024)} MB`);
      return true;
    } catch (error) {
      if (error.message.includes('Espacio insuficiente')) {
        throw error;
      }
      log.warn('No se pudo verificar espacio en disco:', error.message);
  ...
  async downloadResources(onProgress, onStatus) {
    return new Promise((resolve, reject) => {
      ...
      // Verificar espacio disponible antes de descargar (~272 MB + overhead)
      const requiredSpace = 272 * 1024 * 1024; // 272 MB en bytes
      this.checkDiskSpace(requiredSpace).then(hasSpace => {
        if (!hasSpace) {
          reject(new Error('Espacio insuficiente en disco para descargar recursos'));
          return;
        }
  ...
  handleDownload(response, file, onProgress, onStatus, resolve, reject) {
    ...
    file.on('error', (error) => {
      ...
      file.close(); // Cerrar stream explícitamente
      try {
        fs.unlinkSync(this.tempPath);
      } catch (e) {
        // Ignorar error de limpieza
  ...
  async extractZip(onStatus) {
    try {
      ...
      // Si fonts no está en el ZIP, copiar desde recursos locales (solo en desarrollo)
      if (!fs.existsSync(fontsPath)) {
        if (!app.isPackaged) {
          ...
          log.warn('Fonts no encontradas en ZIP. En producción, las fonts deben estar incluidas en el ZIP de recursos.');
        }
      }
